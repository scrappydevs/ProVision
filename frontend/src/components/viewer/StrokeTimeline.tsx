  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="relative h-16 bg-[#1C1A19] rounded-lg overflow-hidden">
          ...
          {strokes.map((stroke) => {
            ...
            return (
              <div
                key={stroke.id}
                className={`absolute top-2 bottom-2 rounded cursor-pointer transition-all hover:opacity-80 ${getScoreColor(stroke.form_score)} ${isActive ? "ring-2 ring-foreground" : ""}`}
                style={{
                  left: `${startPercent}%`,
                  width: `${widthPercent}%`,
                }}
        ...
        {/* Legend */}
        <div className="flex items-center gap-4 text-xs text-muted-foreground pt-2 border-t border-[#2C2A29]">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="text-foreground font-medium">{strokeSummary.total_strokes}</span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground">Forehands</span>
              <span className="text-foreground">{strokeSummary.forehand_count}</span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground">Backhands</span>
              <span className="text-foreground">{strokeSummary.backhand_count}</span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground">Serves</span>
              <span className="text-foreground">{strokeSummary.serve_count}</span>
            </div>
          </div>
