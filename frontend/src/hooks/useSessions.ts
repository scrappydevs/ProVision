...
export function useUpdateSession(sessionId: string) {
  ...
  return useMutation({
    mutationFn: async (data: { camera_facing?: "auto" | "toward" | "away" }) => {
...
/**
 * Track ball trajectory for a session (TrackNet fallback).
 */
export function useTrackObject(sessionId: string) {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async (data: { frame?: number; x?: number; y?: number; detection_box?: number[] }) => {
      const response = await trackWithTrackNet(sessionId, data.frame ?? 0);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: sessionKeys.detail(sessionId) });
...
// ============================================================================
// 3D segmentation queries and mutations
// ============================================================================
...
/**
 * Get segmentation result for a session/object
 */
