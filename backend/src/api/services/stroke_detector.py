...
class StrokeDetector:
    ...
    def _calculate_stroke_metrics(self, stroke_frames: List[Dict], peak_idx: int) -> Dict[str, float]:
        """Calculate metrics for a stroke using the dominant hand's joints."""
        metrics = {}
        dom = self.handedness
        off = "left" if dom == "right" else "right"
    ...
    def _classify_stroke_type(self, stroke_frames: List[Dict], metrics: Dict[str, float]) -> str:
        """
        Classify the stroke type based on body position, movement, and handedness.

        For a right-handed player:
          - Forehand: shoulder rotating left (negative), wrist stays on right side
          - Backhand: shoulder rotating right (positive), wrist crosses to left side

        For a left-handed player: directions are mirrored.
        """
        shoulder_rotation = metrics.get('shoulder_rotation', 0)
        hip_rotation_range = metrics.get('hip_rotation_range', 0)

        if hip_rotation_range > 30:
            return 'serve'

        # Flip interpretation based on handedness
